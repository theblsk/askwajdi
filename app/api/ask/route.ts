...
export async function POST(request: NextRequest) {
  try {
    ...
    if (question.trim().length < 10) {
      return NextResponse.json({ error: "Question must be at least 10 characters long" }, { status: 400 })
    }

    // Send email via Resend
    const emailData = await resend.emails.send({
      from: "AskWajdi <noreply@askwajdi.com>", // Replace with your verified domain
      to: ["wajdi@example.com"], // Replace with Wajdi's actual email
      replyTo: email,
      subject: `New Question from ${name || "Anonymous"} - AskWajdi`,
      html: `
